...
def upgrade() -> None:
    ...
    sa.Column('query_date', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('saved_datasets', sa.Column('basket_id', sa.INTEGER(), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'saved_datasets', type_='foreignkey')
    op.create_foreign_key('saved_datasets_user_id_fkey', 'saved_datasets', 'users', ['user_id'], ['id'], ondelete='CASCADE')
