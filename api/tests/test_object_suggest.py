...
async def test_object_suggest_merge_and_cache(app, client, monkeypatch):
    ...
    async def fake_simbad(q, limit): return [{"service": "SIMBAD", "name": "M 31"}, {"service": "SIMBAD", "name": "M 32"}]
    async def fake_ned(q, limit):    return [{"service": "NED",    "name": "M31"},   {"service": "NED",    "name": "M33"}]
