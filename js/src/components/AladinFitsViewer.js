...
export default function AladinFitsViewer({
  ...
  useEffect(() => {
    ...
    initPromise
      .then(() => {
        ...
        // re/create viewer if first time or survey changed
        ...
        aladin.displayFITS(fitsUrl, (ra, dec, fov, image) => {
          if (cancelled) return;
          ...
          try {
            ...
            if (image?.setColormap) {
              image.setColormap(colormap, { stretch, reversed: false });
            }
  ...
  }, [fitsUrl, divId, colormap, stretch, cuts, baseSurvey]);
